...
There are 3 designated organisers & judges for the event, with varying levels of involvement. The intent behind this structure is to minimise the time required by multiple senior resources, while still providing a support structure to the primary organiser and preventing decision impasses. All three judges are expected to understand the problem well, and be familiar with the possible approaches to solving it.

### Primary Judge

The principal event organiser, problem designer, and judge for solution submissions. This person is expected to perform all possible organisational duties within their power, requesting assistance from external operational resources where necessary.

### Secondary Judge

A secondary problem designer and an equal judge for solution submissions. They are not expected to perform organisational duties, but are required to be present at the kickoff event.

### Tertiary Judge

A tie-breaker for any organisational or judging decisions, where needed. They need only have minimal participation in the event, and need not be present for the kickoff event.

## Structure

The structure of a challenge week is as follows:
...
The new pairs will reattempt the same problem, this time bringing together their knowledge from the previous iteration. For this round, a working solution is desired but there is no consequence for failing to meet this expectation. This may be especially true in the case of more difficult challenges.
